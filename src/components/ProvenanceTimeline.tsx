  ...
  return (
    <div className="space-y-4">
      ...
      {sale_history.length === 0 ? (
        <p className="text-sm text-text-secondary italic">No sales history yet</p>
      ) : (
            ...
            <div key={entry.id} className="relative">
              ...
              <div className="space-y-1">
                <p className="text-xs text-text-secondary">
                  {i === 0 && entry.seller_wallet === artist_wallet ? "Minted" : entry.date}
                </p>
                <p className="text-sm">
                  <span className="font-mono text-xs text-text-secondary">{truncateWallet(entry.seller_wallet)}</span>
                  <span className="mx-2 text-accent">→</span>
                  <span className="font-mono text-xs text-text-secondary">{truncateWallet(entry.buyer_wallet)}</span>
                </p>
                <p className="text-sm font-medium">{formatPrice(entry.price, entry.currency)}</p>
              </div>
            </div>
          ))}
        </div>
